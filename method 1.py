...
class MicrophoneStream(object):
    ...
    def __init__(self, rate, chunk,settings):
        self._rate = rate
        self._chunk = chunk
        self.source = settings["translate_source_language"]
        self.target = settings["translate_target_language"]
        self.seq_count=settings["seq_count"]
...
def listen_print_loop(responses,token,stream):
    ...
    for response in responses:
        ...
        if not result.is_final:
            ...
            start=time.time()
            result=requests.post(token,params=post_params, data=sentence.encode('utf-8'),headers=headers)
            end=time.time()
            print(f"傳送zoom秒數：{end-start}")
...
def main():
    ...
    token=input("請輸入zoom第三方憑證：")
    with open('settings.json') as config:
        settings = json.load(config)
    phrases = ["黃冠為","蕙君","中央長老團","會眾","曾浩一","第六分區"]
    boost = 20.0
    ...
    config = speech.RecognitionConfig(
        ...
        language_code=settings["translate_source_language"],
    ...
    print("請開始說話：")
    with MicrophoneStream(RATE, CHUNK,settings) as stream:
